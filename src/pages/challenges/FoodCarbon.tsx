...
const FoodCarbon: React.FC = () => {
  ...
  const groupByMeal = () => {
    ...
    mealTypes.forEach(meal => {
      meals[meal.id] = foodItems.filter(item => item.name.split(' ')[1] === meal.name);
    });
  ...
  const calculateFootprintByMeal = (mealId: string) => {
    return foodItems
      .filter(item => item.name.split(' ')[1] === mealId)
          ...
          <div className="card-body text-center">
            ...
            <div className="card mb-4">
              ...
              <div className="card-body">
                <div className="row">
                  {mealTypes.map(meal => {
                    const footprint = calculateFootprintByMeal(meal.name);
                    const mealItems = foodItems.filter(item => item.name.split(' ')[1] === meal.name);
                            ...
                            <div className="row mb-3">
                              <div className="col-6">
                                ...
                                <select
                                  ...
                                  value={selectedFood.name.split(' ')[1]}
                                  onChange={(e) => {
                              ...
                              <div className="col-6">
                                ...
                                <small className="text-muted d-block mt-1">
                                  Total: {(selectedFood.carbonFootprint * selectedFood.quantity).toFixed(2)} kg CO₂
                                </small>
                              </div>
                            </div>
                                      ...
                                      <div>
                                        ...
                                        <div>
                                          <small className="text-muted">
                                            {formatDate(new Date(item.name.split(' ')[2]))}
                                          </small>
                                        </div>
                                      </div>
                      ...
                      <table className="table mb-0">
                        ...
                        <tbody>
                          {foodItems.map((item, index) => {
                            ...
                            const meal = mealTypes.find(m => m.name === item.name.split(' ')[1])!;
                            
                            return (
                              <tr key={index}>
                                <td>
                                  <div>
                                    {foodCategories.find(cat => cat.id === item.name.split(' ')[0])?.icon} {item.name.split(' ')[0]}
                                  </div>
                                  <small className="text-muted">
                                    {formatDate(new Date(item.name.split(' ')[2]))}
                                  </small>
